...
Calculator::Calculator() : operand1(0), operand2(0), state("first_number") { }

void Calculator::pressDigit(int digit) {
  if (state == "first_number") {
    // add digit to the end of operand1
    operand1 = operand1 * 10 + digit;
  } else {
    // add digit to the end of operand2
...
void testCalculator1() {
  ...
  c.pressDigit(5);
  c.pressDigit(5);
  assertTrue(c.pressEnter() == (42 + 55), "42 + 55");
}
...
void testCalculator2() {
  ...
  c.pressDigit(1);
  c.pressDigit(2);
  assertTrue(c.pressEnter() == (89 - 12), "89 - 12");
}
