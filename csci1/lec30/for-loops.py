L = []
# fill L with numbers that the user enters
nextNum = int(input("Enter the next number or -1: "))
while nextNum != -1:
    # add nextNum into the list L
    L = L + [nextNum]
    # get the next number
    nextNum = int(input("Enter the next number or -1: "))
...
for x in L:
    # if x is a better guess than min, min should be set to x
    if x < min:
        min = x
...
for n in range(32, 38):  # this'll let us go between 32 and 37
    if isprime(n):
